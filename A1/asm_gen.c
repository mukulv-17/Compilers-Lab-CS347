...
int byte_no;    //smallest empty memory location
FILE *fout;
//stacks for if and while to handle the nested cases
// These store the file positions of beginIf's and, beginWhile's
int if_stck[20],if_stck_top=0,while_stck[20],while_stck_top=0,cond_stck[20],cond_stck_top=0;

char symbol_table[10][10];        //at max 10 variables, each of length at max 10
...
void while_cond()
{
    cond_stck[cond_stck_top++]=byte_no;
...
void gt(int i, int j)               //strictly >
{
    ...
    fprintf(fout, "SUB B\n");
    fprintf(fout, "MVI A, 00H\n");
    fprintf(fout, "JM ");           //move to STA line
    hex_print(byte_no+19, 4);
    fprintf(fout, "JZ ");
    hex_print(byte_no+19, 4);
    fprintf(fout, "MVI A, 01H\n");
...
void divi(int i, int j)
{
    ...
    fprintf(fout, "JC DIVLOOP%d    ;Jump on carry to LOOP\n",divLoopNo); 
    ...
    fprintf(fout, "DIVLOOP%d: MOV A, C        ;Move Content of C - Reg to A - Reg\n",divLoopNo++);
...
//utility function -
int loadVarsFor(char* line, char * substring,int *i,int *j){
    if(strstr(line, substring)!=NULL){
        *i = *(strstr(line, substring) - 1) - '0';
        *j = *(strstr(line, substring) + strlen(substring) + 1) - '0';
        return 1;
    }
    else return 0;        
}
...
int check(char *line)
{
    ...
    if(strstr(line, " if ")!=NULL)
    {
        ...
        //call if1 and pass the temp_var number in its condition
        if1(i);
        return IF;
    }
    else if(strstr(line, " while ")!=NULL)
    {
        ...
        //call while1 and pass the temp_var number in its condition
        while1(i);
        return WHILE;
    }
    else if(strstr(line, " endWhile ")!=NULL)
    {
        ...
        while_end();
        return 100;// return non-zero value to depict no error
    }
    ...
    else if(strstr(line, " beginWhile ")!=NULL) return 100;// return non-zero value to depict no error
    else if(strstr(line, " beginIf ")!=NULL) return 100;         //just some non-zero value
    //ignore the rest
    else if(strstr(line, " then ")!=NULL) return THEN;
    else if(strstr(line, " do ")!=NULL) return DO;
    else if(strstr(line, " begin ")!=NULL) return BEGIN;
    else if(strstr(line, " end ")!=NULL) return END;
    else if(loadVarsFor(line," += ",&i,&j) == 1){//ti += tj
        plus(i, j);
        return PLUS;
    }
    else if(loadVarsFor(line," -= ",&i,&j) == 1){//ti -= tj
    ...
    */
        // if(line[1] == '_')
        if(strstr(line, " _")!=NULL && (strstr(line, "_") < strstr(line, ":=")) )
        {
            //_id := ti
            j = *(strstr(line, " := ") + 5) - '0';
            char sub[20];
            
            //copy variable name with underscore - 
            char* var_start = strstr(line, " _") + 1;
            int var_length = strstr(line, " := ") - var_start;
            strncpy(sub, var_start , var_length );
            sub[var_length]='\0';
            // dprintf(">> stored '_id' = '%s'\n",sub);
